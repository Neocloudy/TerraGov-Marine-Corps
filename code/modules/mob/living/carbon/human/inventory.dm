...
/mob/living/carbon/human/proc/has_limb(org_name)
	if(!(org_name in GLOB.human_body_parts))
		CRASH("[type]/has_limb called with an invalid limb ([org_name])")
	for(var/X in limbs)
		var/datum/limb/E = X
...
/mob/living/carbon/human/proc/has_limb_for_slot(slot)
	switch(slot)
		if(SLOT_BACK)
			return has_limb(BODY_ZONE_CHEST)
		if(SLOT_WEAR_MASK)
			return has_limb(BODY_ZONE_HEAD)
		if(SLOT_HANDCUFFED)
			return has_limb(BODY_ZONE_PRECISE_L_HAND) && has_limb(BODY_ZONE_PRECISE_R_HAND)
		if(SLOT_L_HAND)
			return has_limb(BODY_ZONE_PRECISE_L_HAND)
		if(SLOT_R_HAND)
			return has_limb(BODY_ZONE_PRECISE_R_HAND)
		if(SLOT_BELT)
			return has_limb(BODY_ZONE_CHEST)
		if(SLOT_WEAR_ID)
			return TRUE
		...
		if(SLOT_HEAD)
			return has_limb(BODY_ZONE_HEAD)
		if(SLOT_SHOES)
			return has_limb(BODY_ZONE_PRECISE_R_FOOT) && has_limb(BODY_ZONE_PRECISE_L_FOOT)
		if(SLOT_WEAR_SUIT)
			return has_limb(BODY_ZONE_CHEST)
		if(SLOT_W_UNIFORM)
			return has_limb(BODY_ZONE_CHEST)
