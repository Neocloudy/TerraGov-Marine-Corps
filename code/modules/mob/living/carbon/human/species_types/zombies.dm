...
/datum/species/zombie/handle_unique_behavior(mob/living/carbon/human/H)
	...
	for(var/datum/limb/limb AS in H.limbs) //Regrow some limbs
		if(limb.limb_status & LIMB_MISSING && !(limb.parent?.limb_status & LIMB_MISSING) && prob(10))
			limb.remove_limb_flags(LIMB_MISSING)
			...
			H.update_body()
		else if(limb.limb_status & LIMB_FRACTURED && prob(20))
			limb.remove_limb_flags(LIMB_FRACTURED | LIMB_SPLINTED | LIMB_STABILIZED)
