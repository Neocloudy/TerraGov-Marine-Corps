...
/client/verb/looc(msg as text)
	...
	for(var/client/recv_staff AS in GLOB.admins)
		...
		if(is_mentor(recv_staff) && !isobserver(recv_staff.mob))
			continue // Mentors can hear this from anywhere but only as a ghost
		if(recv_staff.mob == mob)
			continue

		if(recv_staff.prefs.toggles_chat & CHAT_LOOC)
			to_chat(recv_staff, span_looc_heard_staff("<span class='prefix'>[span_tooltip("You are receiving this because you are staff.", "LOOC")]: [ADMIN_TPMONTY(mob)]: [span_message("[msg]")]"))
