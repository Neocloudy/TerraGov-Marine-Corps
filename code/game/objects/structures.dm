	...
	for(var/mob/living/M in get_turf(src))
		...
		if(prob(25))
			...
			switch(pick(list("ankle","wrist","head","knee","elbow")))
				if("ankle")
					affecting = H.get_limb(pick(BODY_ZONE_PRECISE_L_FOOT, BODY_ZONE_PRECISE_R_FOOT))
				if("knee")
					affecting = H.get_limb(pick(BODY_ZONE_L_LEG, BODY_ZONE_R_LEG))
				if("wrist")
					affecting = H.get_limb(pick(BODY_ZONE_PRECISE_L_HAND, BODY_ZONE_PRECISE_R_HAND))
				if("elbow")
					affecting = H.get_limb(pick(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM))
				if("head")
