...
/datum/scanner_advice/damage/brute/get_data(mob/living/carbon/human/patient, mob/user)
	...
	if(!(patient.species.species_flags & (IS_SYNTHETIC|ROBOTIC_LIMBS)))
		. += list(list(
			...
			ADVICE_TOOLTIP = "Trauma kit healing scales with how proficient you are in the Medical field. Treated wounds slowly heal on their own.",
...
/datum/scanner_advice/damage/burn/get_data(mob/living/carbon/human/patient, mob/user)
	...
	if(!(patient.species.species_flags & (IS_SYNTHETIC|ROBOTIC_LIMBS)))
		. += list(list(
			...
			ADVICE_TOOLTIP = "Burn kit healing scales with how proficient you are in the Medical field. Treated wounds slowly heal on their own.",
			ADVICE_ICON = FA_ICON_KIT_MEDICAL,
...
/datum/scanner_advice/damage/tox/get_data(mob/living/carbon/human/patient, mob/user)
	. = list(
		...
		ADVICE_TOOLTIP = "Significant blood toxins detected. Dylovene will purge toxins and toxin damage, or the liver may filter it out with time.",
		ADVICE_ICON = FA_ICON_CAPSULES,
...
/datum/scanner_advice/damage/oxy/get_data(mob/living/carbon/human/patient, mob/user)
	. = list(
		...
		ADVICE_TOOLTIP = "If you don't have these chemicals, CPR or treating their other symptoms and waiting for Oxy damage to go down will work.",
...
/datum/scanner_advice/damage/clone/get_data(mob/living/carbon/human/patient, mob/user)
	...
	. = list(
		...
		ADVICE_TOOLTIP = "[organic_patient ? "Cellular damage" : "Integrity damage"] is sustained from psychic draining, special chemicals and special weapons.",
